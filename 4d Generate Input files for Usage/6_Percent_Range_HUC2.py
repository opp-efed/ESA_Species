...
in_raster_location = r'L:\ESA\UnionFiles_Winter2018\Range\SpComp_UsageHUCAB_byProjection_2\Grid_byProjections_Combined'
...
def melt_df(df_melt):
    ...
    sum_by_ent = df_melt_row.groupby(['EntityID', 'HUC2_AB', 'STUSPS', 'STATEFP']).sum()
...
def parse_tables(in_table, in_row_sp):
    ...
    merged_df['STATEFP'] = merged_df['GEOID'].map(lambda (n): str(n)[:2] if len(n) == 5 else '0' + n[:1]).astype(str)
...
for folder in list_folder:
    ...
    for raster in list_raster:
        if os.path.exists(outlocation+os.sep+raster+date+'.csv'):
            ...
            raster_zone_df['HUCID'] = raster_zone_df['VALUE'].map(lambda x: str(x).split('.')[0]).astype(str)
            sp_zone_fc = [j for j in list_fc if j.startswith(file_type + raster.split("_")[1].title())]
            par_zone_fc = [j for j in list_fc_ab if j.startswith(file_type + raster.split("_")[1].title())]
            ...
            merg_df = pd.merge(raster_zone_df,par_zone_df, on='HUCID', how='left')
            use_array = parse_tables(merg_df, sp_zone_df)
            print use_array.columns.values()
...
concat_df.to_csv(outpath+os.sep+file_type+'HUC2_Acres_Pixels_'+date+'.csv')
