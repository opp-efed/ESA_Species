...
table_folder = 'C:\Users\JConno02\Documents\Projects\ESA\Bins\updates\Script_Check_20170627'

# INPUT SOURCES
# Current_Bins_tobeLoadDB_[date].csv from Step_2_UpdateBinTable_BinAssignments; if not feedback on bin assignments have
# been received copy UpdatedBins_[date].csv  from Step_1_UpdateBinTable_RangeUpdate from archive folder to table_folder
# path and use that table
wide_bin_coded_name = 'Current_Bins_tobeLoadDB_20170627.csv'
...
# ## All bin assignment that starts with a 13 indicated a bin the species is found in when the HUC is coastal, however
# this bin does not occur in land locked areas.
...
wide_bin_coded = table_folder + os.sep + wide_bin_coded_name
...
# Step 4: Merge the species info, bins and database info back into one df, concat no common col, axis=1 merged along col
# index
...
# Recode_BinTable_as_of_[date].csv  # working with shorthand value and not codes - removed dropped species
# Recode_BinTable_as_of_unfiltered_[date].csv  # working with shorthand value and not codes - all species
df = df_final.loc[df_final[str(entity_id_col_c)].isin(list_sp)]
df_final.to_csv(outfile_unfiltered, encoding='utf-8')
