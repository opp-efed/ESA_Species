...
infolder = 'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TimMcNest\Results_Fall2016\Agg_layers\NonAg'
##export att table of refuge clips of hab
outlocation = 'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TimMcNest\Tables_Fall2016\Agg_layers\NonAg\SumBySpecies'
completedlist = []
...
def summarize_species(listfolder,multi,multi_break):
    ...
    for folder in listfolder:
        ...
        use = useLookup[use]
        list_all_csv = os.listdir(infolder+ os.sep + folder)
        ...
        if multi:
            ...
            if multi_break == '':
                csv_list_species=[csv for csv in csv_list_species if str(csv.split("_")[6]) == 'CONUS']
            else:
                csv_list_species = [csv for csv in csv_list_species if str(csv.split("_")[6]) == multi_break]
        print csv_list_species
        for csv in csv_list_species:

            out_use_df = pd.DataFrame()
            in_use_df = pd.read_csv((infolder + os.sep + folder+ os.sep+ csv), dtype=object)
