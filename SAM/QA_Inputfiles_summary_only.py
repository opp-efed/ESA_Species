...
input_file_location = r'P:\GIS_Data\SAM\scenarios_and_recipes'
master_input_table = r'C:\Users\JConno02\OneDrive - Environmental Protection Agency (EPA)\Documents_C_drive\Projects\SAM' \
                     r'\ScenarioInputs_par_20171004.csv'
outpath = r'C:\Users\JConno02\OneDrive - Environmental Protection Agency (EPA)\Documents_C_drive\Projects\SAM'
...
dtype_dict = {'scenario': str, 'mukey': str, 'cokey': str, 'state': str, 'cdl': int, 'weatherID': int, 'date': str,
              ...
              'uslels': float, 'RZmax': int, 'sfac': float, 'rainfall': float, 'anetd': float, 'plntbeg': int,
              ...
              'cfact_cov': float, 'ManningsN': float, 'uslep': float, 'plntbegAct': int,'plntendAct': int,
              'hrvbegAct': int,'hrvendAct': int}
...
for in_file in list_input:
    ...
    outfile_yellow = outpath + os.sep + 'QA_'+ str(state)+'_Errors_SAM_yellow_' + str(date) + '.csv'
    outfile_red = outpath + os.sep + 'QA_'+ str(state)+'_Errors_SAM_red_' + str(date) + '.csv'
    outfile_other = outpath + os.sep + 'QA_'+ str(state)+'_Errors_SAM_other_' + str(date) + '.csv'
    outfile_summary = outpath + os.sep + 'QA_'+ str(state)+'_Errors_SAM_summary_' + str(date) + '.csv'
    ...
    if not os.path.exists(outfile_red):
        ...
        print '\nChecking variables with general ranges...'  # Yellow Flag
        ...
        print '\nChecking variables with discrete values...' #  Red Flag
        ...
        del merged_df, out_df, summary_df,out_red, out_yellow, out_other, c_input, working_df
