...
def loop_species(group):
    ...
    df_group = masterlist_df.loc[(masterlist_df[group_colindex] == group)].copy()
...
for group in alpha_group:
    ...
    if totalfc == total_ingroup:
        ...
        if len(removed) > 0:
            print "species {0} where removed before overlap; Range not created or excluded from overlap".format(removed)
