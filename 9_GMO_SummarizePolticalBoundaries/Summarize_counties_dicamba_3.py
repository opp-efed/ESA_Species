...
all_crop_df['County confidence'] = np.where(((all_crop_df['Percent of County']>10)&
                                             (all_crop_df['Percent of Species']>percent_spe_overlap_in_cnty)&
                                             ((all_crop_df['Percent_Cnty_CoA_Soybeans_0']>0.45) &
                                              (all_crop_df['Percent_Cnty_UDL_Soybeans_0']>0.45)|
                                              (all_crop_df['Percent_Cnty_CoA_Cotton_0']>0.45) &
                                              (all_crop_df['Percent_Cnty_UDL_Cotton_0']>0.45))), 'Include', 'Not Confident')
...
for v in list_intervals:
    ...
    sp_range =species_df[(species_df ['File Type'] =='Range') &((species_df['adjusted on_off_Soybeans_'+str(v)]>0.45)|species_df['adjusted on_off_Cotton_'+str(v)]>0.45)].copy()
    range_list = list(set(sp_range['EntityID'].values.tolist()))
    sp_ch =species_df[(species_df ['File Type'] !='Range')&((species_df['adjusted on_off_Soybeans_'+str(v)]>0.45)|species_df['adjusted on_off_Cotton_'+str(v)]>0.45)].copy()
    ch_list = list(set(sp_range['EntityID'].values.tolist()))
    # all_crop_df= all_crop_df[all_crop_df['EntityID'].isin(range_list)].copy()
    ...
    non_monocot =  all_crop_df [((all_crop_df['Plant taxa'] !='M')&(all_crop_df ['WoE Summary Group'] =='Plants')&
                                (all_crop_df['adjusted on_off_Soybeans_' + str(v)]>=1)&
                                 ...
                                 (all_crop_df['Percent_Cnty_Soybeans_' + str(v)]>=1)&
                                ...
                                ((all_crop_df['Plant taxa'] !='M')&(all_crop_df ['WoE Summary Group'] =='Plants')&
                                 ...
                                 (all_crop_df['Percent_Cnty_Cotton_' + str(v)]>=1)&
                                (all_crop_df ['File Type'] =='Range')&
                                (all_crop_df ['EntityID'].isin(range_list))&(all_crop_df['Percent of Species']>percent_spe_overlap_in_cnty))|
    ...
    non_monocot['County confidence'] = np.where(((non_monocot['Percent of County']>10)&
                                                 (non_monocot['Percent of Species']>percent_spe_overlap_in_cnty)&
                                                 ((non_monocot['Percent_Cnty_Full_Soybeans_'+str(v)]>=percent_cnty_crop)|
    ...
    non_monocot['UDL/CoA Soybeans'] = (non_monocot['Soybeans_0_acres']/non_monocot['SOYBEANS']) * 100
    non_monocot['UDL/CoA Cotton'] = (non_monocot['Cotton_0_acres']/non_monocot['COTTON'])  * 100
    non_monocot['Percent of Total UDL Soybean'] = (non_monocot['Soybeans_0_acres']/non_monocot['Acres_UDL_SOYBEANS_TOTAL']) * 100
    non_monocot['Percent of Total UDL Cotton'] = (non_monocot['Cotton_0_acres']/non_monocot['Acres_UDL_Cotton_TOTAL'])  * 100
    # non_monocot['Percent Impact UDL Cotton'] = (percent_calc ['Cotton_0_acres'].sum()/percent_calc ['COTTON_UDL_Total'])*100
    # non_monocot['Percent Impact UDL Soybean'] = (percent_calc ['Soybeans_0_acres'].sum()/percent_calc ['SOYBEAN_UDL_Total'])*100
    ...
    non_monocot_obl =  all_crop_df [((all_crop_df['Plant taxa'] !='M')&(all_crop_df ['WoE Summary Group'] =='Plants')&
                                ...
                                ((all_crop_df ['WoE Summary Group'].isin(terr_woe_groups))
                                 ...
                                 (all_crop_df['Percent_Cnty_Cotton_' + str(v)]>=1)
                                 &(all_crop_df ['File Type'] !='Range') &
                                 (all_crop_df ['EntityID'].isin(ch_list)))|((all_crop_df ['obligate plant']=='Yes')&
                                                                                                                 (all_crop_df['adjusted on_off_Soybeans' + "_"+str(v)]>=1)&
                                                                                                                 (all_crop_df ['File Type'] =='Range'))
    ...
    # non_monocot_obl['Percent Impact UDL Cotton'] = (percent_calc ['Cotton_0_acres'].sum()/percent_calc ['COTTON_UDL_Total'])*100
    # non_monocot_obl['Percent Impact UDL Soybean'] = (percent_calc ['Soybeans_0_acres'].sum()/percent_calc ['SOYBEAN_UDL_Total'])*100
    non_monocot_obl.to_csv(out_path_combined + os.sep + 'Combined_NonMonocotObl_' + str(v) + '.csv')


    generalist =  all_crop_df[((all_crop_df['adjusted on_off_Soybeans' + "_"+str(v)]>=1)& (all_crop_df['SOYBEANS']>=1)&
                                   (all_crop_df ['File Type'] =='Range'))
                                  |((all_crop_df['adjusted on_off_Cotton' + "_"+str(v)]>=1)& (all_crop_df['COTTON']>=1)&
                                    (all_crop_df ['File Type'] =='Range'))|
                                  ((all_crop_df ['WoE Summary Group'].isin(terr_woe_groups))
                                   &(all_crop_df['adjusted on_off_Soybeans' + "_"+str(v)]>=1)
                                   & (all_crop_df['SOYBEANS']>=1)
    ...
    generalist['County Count'] = generalist['GEOID'].nunique()
    generalist['County confidence'] = np.where((
                                                (( generalist['Percent_Cnty_Full_Soybeans_'+str(v)]<percent_cnty_crop)&
                                                 ( generalist['Percent_Cnty_Full_Cotton_'+str(v)]<percent_cnty_crop))),
        'Not Confident','Include', )

    percent_calc = generalist[generalist['County confidence']=='Include'].copy()
    percent_calc = percent_calc[['GEOID','COTTON','COTTON_Total','SOYBEANS','SOYBEAN_Total']].copy()
    percent_calc.drop_duplicates(inplace=True)
    percent_calc['Confident County Count'] = percent_calc['GEOID'].nunique()
    percent_calc['Percent Impact Cotton'] = (percent_calc ['COTTON'].sum()/percent_calc ['COTTON_Total'])*100
    ...
    generalist= pd.merge(generalist,percent_calc,how='left',on='GEOID')
    # generalist['Percent Impact UDL Cotton'] = (percent_calc ['Cotton_0_acres'].sum()/percent_calc ['COTTON_UDL_Total'])*100
    # generalist['Percent Impact UDL Soybean'] = (percent_calc ['Soybeans_0_acres'].sum()/percent_calc ['SOYBEAN_UDL_Total'])*100
    generalist.to_csv(out_path_combined + os.sep + 'Generalist_Combined_' + str(v) + '.csv')

    all_ch =  all_crop_df[((all_crop_df['adjusted on_off_Soybeans' + "_"+str(v)]>=1)& (all_crop_df['SOYBEANS']>=1) &
                           (all_crop_df ['File Type'] =='Range')&(all_crop_df['Percent of Species']>percent_spe_overlap_in_cnty))
                          |((all_crop_df['adjusted on_off_Cotton' + "_"+str(v)]>=1)& (all_crop_df['COTTON']>=1)&
                            (all_crop_df ['File Type'] =='Range')&(all_crop_df['Percent of Species']>percent_spe_overlap_in_cnty))|((all_crop_df['adjusted on_off_Soybeans' + "_"+str(v)]>=1)& (all_crop_df['SOYBEANS']>=1)
                                                                    &(all_crop_df ['File Type'] !='Range'))|((all_crop_df['adjusted on_off_Cotton' + "_"+str(v)]>=1)& (all_crop_df['COTTON']>=1)
                                                                                                             &(all_crop_df ['File Type'] !='Range'))].copy()

    all_ch['County Count'] = all_ch['GEOID'].nunique()
    all_ch['County confidence'] = np.where((
                                            (( all_ch['Percent_Cnty_Full_Soybeans_'+str(v)]<percent_cnty_crop)&
                                             ( all_ch['Percent_Cnty_Full_Cotton_'+str(v)]<percent_cnty_crop))),
     'Not Confident', 'Include')
    ...
    all_ch= pd.merge(all_ch,percent_calc,how='left',on='GEOID')
    # all_ch['Percent Impact UDL Cotton'] = (percent_calc ['Cotton_0_acres'].sum()/percent_calc ['COTTON_UDL_Total'])*100
    # all_ch['Percent Impact UDL Soybean'] = (percent_calc ['Soybeans_0_acres'].sum()/percent_calc ['SOYBEAN_UDL_Total'])*100
    all_ch.to_csv(out_path_combined + os.sep + 'Generalist_CH_Combined_' + str(v) + '.csv')
