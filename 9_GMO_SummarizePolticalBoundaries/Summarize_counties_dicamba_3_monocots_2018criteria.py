...
out_path = 'E:\Dicamba Update Summer 2020\Update Summer 2020\Test_Overlap Summary by Crop_onoff_wcntysp_old'
...
coa['GEOID'] = coa['GEOID'].map(lambda x: str(x).split('.')[0] if len((str(x).split('.')[0])) == 5 else '0' + (str(x).split('.')[0])).astype(str)
...
for i in path_table:
    ...
    if '20x2' in os.path.basename(i).split('_'):
        crop = 'Cotton'
        udl_cnty = r"E:\Dicamba Update Summer 2020\Update Summer 2020\CONUS_CDL_1317_20x2_euc_County.csv"
        udl_col = 'Acres_UDL_Cotton_CNTY_TOTAL'
    else:
        crop = 'Soybeans'
        udl_cnty = r"E:\Dicamba Update Summer 2020\Update Summer 2020\CONUS_CDL_1317_40x2_euc_County.csv"
        udl_col = 'Acres_UDL_SOYBEANS_CNTY_TOTAL'
...
all_crop_df['County confidence'] = np.where(((all_crop_df['Percent of Species']>percent_cnty_crop)), 'Include', 'Not Confident')
print list(set(all_crop_df['File Type'].values.tolist()))
# all_crop_df['EntityID'] = all_crop_df ['EntityID'].map(lambda x: str(x).split(".")[0]).astype(str)
...
for v in list_intervals:
    ...
    print list(set(all_crop_df['File Type'].values.tolist()))
    non_monocot =  all_crop_df [((all_crop_df['Percent of County']>percent_spe_overlap_in_cnty)&
                                 (all_crop_df['Plant taxa'] !='M')&(all_crop_df ['WoE Summary Group'] =='Plants')
                                 &(all_crop_df['adjusted on_off_Soybeans_' + str(v)] >=1)&(all_crop_df['SOYBEANS']>=1)
                                ...
                                ((all_crop_df ['WoE Summary Group'].isin(terr_woe_groups))
                                 ...
                                 &(all_crop_df['Percent_Cnty_Soybeans_' + str(v)]> 0)
                                 &(all_crop_df ['File Type'] !='Range')&(all_crop_df ['EntityID'].isin(ch_list)))
                                 |((all_crop_df ['WoE Summary Group'].isin(terr_woe_groups))
                                   & (all_crop_df['adjusted on_off_Cotton' + "_" + str(v)] >= 1)
                                   & (all_crop_df['COTTON'] >= 1)
                                   & (all_crop_df['Percent_Cnty_Cotton_' + str(v)] > 0)
                                   & (all_crop_df['File Type'] != 'Range')
                                   & (all_crop_df['EntityID'].isin(ch_list)))].copy()

    non_monocot['County Count'] = non_monocot['GEOID'].nunique()
    non_monocot['County confidence'] = np.where((((non_monocot['Percent_Cnty_Full_Soybeans_'+str(v)]>percent_cnty_crop)|
                                                      (non_monocot['Percent_Cnty_Full_Cotton_'+str(v)]>percent_cnty_crop))),
                                                     'Include', 'Not Confident')
    non_monocot['County confidence Split Sliver'] = np.where(((non_monocot['Percent of County'] < 1)|(non_monocot['Percent of Species']<1)),
                                                                 'Sliver', '')

    non_monocot['Percent of Total UDL Soybean'] = (non_monocot['Soybeans_0_acres']/non_monocot['Acres_UDL_SOYBEANS_CNTY_TOTAL']) * 100
    non_monocot['Percent of Total UDL Cotton'] = (non_monocot['Cotton_0_acres']/non_monocot['Acres_UDL_Cotton_CNTY_TOTAL'])  * 100
    non_monocot['County confidence Split in between'] = np.where((((non_monocot['Percent of Total UDL Soybean']>percent_cnty_crop)|
                                                      (non_monocot['Percent of Total UDL Cotton']>percent_cnty_crop))),
                                                    'UDL Overlap', '')

    percent_calc = non_monocot[non_monocot['County confidence']=='Include'].copy()
    percent_calc = percent_calc[['GEOID','COTTON','COTTON_Total','SOYBEANS','SOYBEAN_Total',
                                 'Percent of Total UDL Soybean','Percent of Total UDL Cotton']].copy()
    percent_calc = percent_calc.groupby(['GEOID','COTTON','COTTON_Total','SOYBEANS','SOYBEAN_Total'], as_index=False).max().reset_index()
    ...
    percent_calc['Adjusted COTTON'] = (percent_calc ['COTTON']* (percent_calc ['Percent of Total UDL Cotton']/100))
    percent_calc['Adjusted SOYBEANS'] = (percent_calc ['SOYBEANS']* (percent_calc ['Percent of Total UDL Soybean']/100))
    percent_calc['Adjusted Percent Impact Cotton'] = (percent_calc ['Adjusted COTTON'].sum()/percent_calc ['COTTON_Total'])*100
    percent_calc['Adjusted Percent Impact Soybean'] = (percent_calc ['Adjusted SOYBEANS'].sum()/percent_calc ['SOYBEAN_Total'])*100
    percent_calc= percent_calc[['GEOID','Percent Impact Soybean','Percent Impact Cotton',
                                'Adjusted Percent Impact Cotton',
                                'Adjusted Percent Impact Soybean','Confident County Count']]

    non_monocot = pd.merge(non_monocot,percent_calc,how='left',on='GEOID')
    grouped_df = non_monocot.groupby("GEOID").agg({"EntityID": "nunique"}).reset_index()
    grouped_df.columns = ["GEOID","# of Species in County"]
    non_monocot = pd.merge(non_monocot,grouped_df ,how='left',on='GEOID')
    grouped_df = non_monocot.groupby("EntityID").agg({"GEOID": "nunique"}).reset_index()
    grouped_df.columns = ["EntityID","# of Counties in Species"]
    non_monocot = pd.merge(non_monocot,grouped_df ,how='left',on='EntityID')
    grouped_df = non_monocot.groupby(["EntityID","County confidence"]).agg({"GEOID": "nunique"}).reset_index()
    grouped_df.columns = ["EntityID","County confidence","# of County confidence in Species"]
    non_monocot = pd.merge(non_monocot,grouped_df ,how='left',on=["EntityID","County confidence"])
    non_monocot['County confidence Split Wide Area'] = np.where((((non_monocot['# of Counties in Species']- non_monocot['# of County confidence in Species'])==0)),
                                                    'Wide Area No Counties','')
    non_monocot.to_csv(out_path_combined + os.sep + 'Combined_NonMonocot_' + str(v) + '.csv')
...
for v in list_intervals:
    ...
    non_monocot_obl =  all_crop_df[((all_crop_df['Percent of County']>percent_spe_overlap_in_cnty)&
                         (all_crop_df['Plant taxa'] !='M')&(all_crop_df ['WoE Summary Group'] =='Plants')
                         &(all_crop_df['adjusted on_off_Soybeans_' + str(v)] >=1)&(all_crop_df['SOYBEANS']>=1)
                         &(all_crop_df['Percent_Cnty_Soybeans_' + str(v)]> percent_spe_overlap_in_cnty)&
                         (all_crop_df ['File Type'] =='Range')&
    ...
    non_monocot_obl['County confidence'] = np.where((((non_monocot_obl['Percent_Cnty_Full_Soybeans_'+str(v)]>percent_cnty_crop)|
                                                     ...
                                                     'Include', 'Not Confident')
    non_monocot_obl['County confidence Split Sliver'] = np.where(((non_monocot_obl['Percent of County'] < 1)|(non_monocot_obl['Percent of Species']<1)),
                                                    'Sliver', '')

    non_monocot_obl['Percent of Total UDL Soybean'] = (non_monocot_obl['Soybeans_0_acres']/non_monocot_obl['Acres_UDL_SOYBEANS_CNTY_TOTAL']) * 100
    non_monocot_obl['Percent of Total UDL Cotton'] = (non_monocot_obl['Cotton_0_acres']/non_monocot_obl['Acres_UDL_Cotton_CNTY_TOTAL'])  * 100
    non_monocot_obl['County confidence Split in Between'] = np.where((((non_monocot_obl['Percent of Total UDL Soybean']>percent_cnty_crop)|
                                                      (non_monocot_obl['Percent of Total UDL Cotton']>percent_cnty_crop))),
                                                    'UDL Overlap','')
    ...
    percent_calc['Adjusted Percent Impact Soybean'] = (percent_calc ['Adjusted SOYBEANS'].sum()/percent_calc ['SOYBEAN_Total'])*100
    percent_calc= percent_calc[['GEOID', 'Percent Impact Soybean', 'Percent Impact Cotton',
                                'Adjusted Percent Impact Soybean', 'Adjusted Percent Impact Cotton',
                                'Confident County Count']]

    non_monocot_obl= pd.merge(non_monocot_obl,percent_calc,how='left',on='GEOID')
    grouped_df = non_monocot_obl.groupby("GEOID").agg({"EntityID": "nunique"}).reset_index()
    grouped_df.columns = ["GEOID","# of Species in County"]
    non_monocot_obl = pd.merge(non_monocot_obl,grouped_df ,how='left',on='GEOID')
    grouped_df = non_monocot_obl.groupby("EntityID").agg({"GEOID": "nunique"}).reset_index()
    grouped_df.columns = ["EntityID","# of Counties in Species"]
    non_monocot_obl = pd.merge(non_monocot_obl,grouped_df ,how='left',on='EntityID')
    grouped_df = non_monocot_obl.groupby(["EntityID","County confidence"]).agg({"GEOID": "nunique"}).reset_index()
    grouped_df.columns = ["EntityID","County confidence","# of County confidence in Species"]
    non_monocot_obl = pd.merge(non_monocot_obl,grouped_df ,how='left',on=["EntityID","County confidence"])
    non_monocot_obl['County confidence Split Wide Area'] = np.where((((non_monocot_obl['# of Counties in Species']- non_monocot_obl['# of County confidence in Species'])==0)),
                                                    'Wide Area No Counties', '')
