...
sharepoint_path = r'C:\Users\JConno02\Environmental Protection Agency (EPA)\Endangered Species Pilot Assessments - OverlapTables\ChemicalTables\Carbaryl\Critical Habitat\ParentTables\avg'
col_order = ['Unnamed: 0', 'EntityID', 'STATEFP', 'VALUE_0', 'VALUE_30', 'VALUE_42', 'VALUE_60', 'VALUE_67', 'VALUE_84',
             ...
             'VALUE_1497', 'VALUE_1499', 'VALUE_1500', 'STATE', 'PCT_Carbaryl AA', 'Acres', 'State direct msq',
             'Drift_PCT', 'State msq adjusted by PCT', 'Total outside species range', 'Min in Species range',
             'Max in species range', 'Uniform',
             ]
...
for root,dirs,files in os.walk(sharepoint_path, topdown=False):
    for name in files:
        ...
        if file_name.endswith('.csv'):
            ...
            if col_share == col_order:
                print("Print cols are identical")
            else:
                if not 'noadjust' in file_name.split(os.sep):
                    print("Print cols are not identical")
                    print (list(set(col_share) - set(col_order)))
                    print file_name
                    share_df= share_df.reindex(columns=col_order)
                    print share_df.columns.values.tolist()
                    share_df.to_csv(file_name)
