...
# Description: Generates drift tables that includes the results for all UDLs used for Step 2. Output includes the
...
# treated acres bounding - these can be range or critical habitat tables

# Unadjusted tables
# BE_interval_table = r"CH_UnAdjusted_SprayInterval_noadjust_Full CH_20200812.csv"
BE_interval_table = r"R_UnAdjusted_SprayInterval_noadjust_Full Range_20200812.csv"
...
# be_pct_table = "CH_Uniform_SprayInterval_Full CH_census_avg_20200812.csv"
be_pct_table = "R_Uniform_SprayInterval_Full Range_census_avg_20200812.csv"

# #  PCT, On/Off site adjusted Tables for census - example tables loops over all tables
# # If user doesn't want to include on/off use the same tables as the PCT adjusted
BE_sum_PCT_onoff_interval_table = "R_Uniform_SprayInterval_On OffField_census_avg_20200902.csv"
# BE_sum_PCT_onoff_interval_table = "CH_Uniform_SprayInterval_Full CH_census_avg_20200812.csv"

# PCT, On/Off site adjusted Tables for census habitat elevation or both- example tables loops over all tables
# If user doesn't want to include on/off can leave table name blank

# BE_sup_table = "R_Uniform_SprayInterval_Full Range_adjHab_avg_20200812.csv"
# BE_onoff_sup_table = "R_Uniform_SprayInterval_On OffField_adjHab_avg_20200902.csv"
...
def on_off_field(cols, df, on_off_species, df_not_on_off, acres):
    # Adjust the drift summarized calls for the on/off site cols
...
def extract_chemical_overlap(col_prefix, region, df, distance_bins, section):
...
def no_adjust_tables(interval_df, outpath, conus_cols_load, nl48_cols_load, col_prefix_conus, col_prefix_NL48,
                     base_sp_df):
    ...
    # NL48 into 1
    chemical_noadjustment, col_selection_conus = extract_chemical_overlap(col_prefix_conus, 'CONUS',
                                                                          l48_df, bins, 'no adjustment - CONUS')
    chemical_noadjustment = pd.merge(base_sp_df, chemical_noadjustment, on='EntityID', how='left')
    if nl48:
        out_nl48_df, col_selection_nl48 = extract_chemical_overlap(col_prefix_NL48, 'NL48', nl48_df, bins,
                                                               'no adjustment - NL48')
...
def pct_table(be_sum_pct_interval, conus_cols_load, nl48_cols_load, out_path, group, bound, col_prefix_CONUS,
              bins, col_prefix_NL48, base_sp_df, conus_cols_f, nl48_cols_f, count_on_off):
    ...
    # Loads chemical information needed from no adjustment tables and merges to species list then merges L48 and
    # NL48 into 1
    chemical_pct, col_selection_conus = extract_chemical_overlap(col_prefix_CONUS,  'CONUS', l48_df_pct,
                                                                 bins, 'pct -conus')
    chemical_pct = pd.merge(base_sp_df, chemical_pct, on='EntityID', how='left')
    if nl48:
        out_nl48_df_pct, col_selection_nl48 = extract_chemical_overlap(col_prefix_NL48,  'NL48', nl48_df_pct,
                                                                   bins, 'pct-nl48')
        # merges the CONUS and NL48 tables
...
def redundancy_tables(out_path, group, bound, chemical_pct, conus_cols_f, nl48_cols_f, other_layer_cols, ag_cols,
    ...
    chemical_pct_redundancy.ix[:, overlap_cols_conus] = chemical_pct_redundancy.ix[:, overlap_cols_conus].apply(pd.to_numeric,
                                                                                          errors='coerce')
    if nl48:
        chemical_pct_redundancy.ix[:, overlap_cols_nl48] = chemical_pct_redundancy.ix[:, overlap_cols_nl48].apply(pd.to_numeric,
                                                                                            errors='coerce')
    ...
    # calculated the Non Ag factors for CONUS and NL48 = where clause removed the rows where the  Ag composite is equal
    # to 0
    if not skip_non_ag_adjustment:
    ...
    if nl48:
        ...
        chemical_pct_redundancy = apply_factor(chemical_noadjustment, chemical_pct_redundancy, skip_non_ag_adjustment,
                                               use_direct_only_conus_ag, '', use_direct_only_conus_ag_aa, '',
                                               use_direct_only_conus_nonag, '', use_direct_only_conus_nonag_aa, '',
                                               overlap_cols_conus,'')  # apply factor

    chemical_pct_redundancy.fillna(0, inplace=True)  # fill blanks with 0s
    # final regional tables and exports that are only PCT and census adjusted
    ...
    if nl48:
        ...
        nl48_pct_red_df.to_csv(out_path_redundancy + os.sep + file_type_marker + 'NL48_Step2_Intervals_Redundancy_' +
                               chemical_name + '.csv')
    chemical_pct_redundancy.to_csv(out_path_redundancy + os.sep + file_type_marker + 'Step2_Intervals_Redundancy_' +
                                   chemical_name + '.csv')
    ...
    if nl48:
        ...
        return out_path_redundancy, chemical_pct_redundancy, skip_non_ag_adjustment, use_direct_only_conus_ag, \
               '', use_direct_only_conus_ag_aa, '', use_direct_only_conus_nonag, '', use_direct_only_conus_nonag_aa, \
               '', overlap_cols_conus, '',aa_col_conus, ''
...
def on_off_tables(count_on_off, out_path, group, bound, be_onoff_sup_table, be_sum_PCT_path_sup, be_sum_PCT_path,
                  ...
                  chemical_pct_redundancy, col_prefix_CONUS,  bins, col_prefix_NL48,
    ...
    # Loads chemical information needed from no adjustment tables and merges to species list then merges CONUS and
    # NL48 into 1
    chemical_pct_onoff, col_selection_conus = extract_chemical_overlap(col_prefix_CONUS,  'CONUS',
                                                                       l48_df_pct_onoff, bins, 'pct -conus')
    chemical_pct_onoff = pd.merge(base_sp_df, chemical_pct_onoff, on='EntityID', how='left')
    if nl48:
        out_nl48_df_pct_onoff, col_selection_nl48 = extract_chemical_overlap(col_prefix_NL48, 'NL48',
    ...
    chemical_on_off = apply_factor(chemical_noadjustment, chemical_pct_onoff, skip_non_ag_adjustment,
    ...
    for col in chemical_pct_redundancy_on_off:
        if col not in use_cols and col.endswith("_0") and 'AA' not in col.split(" ") \
                ...
                and 'Open Space Developed'not in col.split("_"):
    ...
    conus_pct_onoff_df.to_csv(
        out_path_on_off + os.sep + file_type_marker + 'CONUS_Step2_Intervals_On_Off_Field_' + chemical_name + '.csv')
    if nl48:
        nl48_pct_onoff_df.to_csv(
        out_path_on_off + os.sep + file_type_marker + 'NL48_Step2_Intervals_On_Off_Field_' + chemical_name + '.csv')
    chemical_pct_redundancy_on_off.to_csv(
        out_path_on_off + os.sep + file_type_marker + 'Step2_Intervals_On_Off_Field_' + chemical_name + '.csv')
    ...
    parameters_used = save_parameter(parameters_used, chemical_name, file_type, use_lookup, master_list,
                                     BE_interval, BE_sum_PCT_interval, be_sum_pct_onoff_interval, out_path_on_off,
                                     count, acres_col)
    count += 1
...
def load_inputs(use_lookup, acres_col, on_off_excel):
    ...
    return use_lookup_df, acres_df, on_off_df, base_sp_df, col_prefix_CONUS,  col_prefix_NL48, skip_non_ag_adjustment,\
           other_layer_cols, ag_cols, nonag_cols
...
def main():
    ...
    parameters_used = pd.DataFrame(
        columns=['Chemical Name', 'File Type', 'Use Lookup', 'Species List','Acres', 'In Location No Adjust',
                 'In Location PCT Adjust','In Location PCT NL48', 'In Location PCT/On Off L48', 'Out file',
                 'Out Base Location'])
    ...
    # load input tables
    use_lookup_df, acres_df, on_off_df, base_sp_df, col_prefix_conus, col_prefix_NL48, skip_non_ag_adjustment, \
    other_layer_cols, ag_cols, nonag_cols = load_inputs(use_lookup, acres_col, on_off_excel)
        ...
        for group in ['avg', 'min', 'max']:  # pct groups
            for bound in ['Lower', 'Upper', 'Uniform']:  # types of bounding scenarios
                ...
                chemical_pct, out_path_pct = pct_table(BE_sum_PCT_interval, conus_cols_load, nl48_cols_load, out_path,
                                                       group, bound, col_prefix_conus,  bins,col_prefix_NL48,
                                                       base_sp_df, conus_cols_f, nl48_cols_f, count_on_off)
                ...
                aa_col_conus,aa_col_nl48 = \
                    redundancy_tables(out_path, group, bound, chemical_pct, conus_cols_f, nl48_cols_f, other_layer_cols,
                                      ag_cols, nonag_cols, chemical_noadjustment,skip_non_ag_adjustment)
                ...
                count += 1
                #
                # Overlap Scenario 4 and 5 - PCT, Redundancy, and On/Off sited adjusted tables; includes supplemented
                # information export if parameters set by users
                ...
                out_path_on_off, parameters_used, count = on_off_tables(count_on_off, out_path, group, bound,
                                                                        ...
                                                                        BE_sum_PCT_onoff_interval_table,
                                                                        parameters_used, count, chemical_noadjustment,
                                                                        chemical_pct_redundancy, col_prefix_conus,
                                                                        bins, col_prefix_NL48, base_sp_df, conus_cols_f,
                                                                        nl48_cols_f, conus_cols_load, nl48_cols_load,
                                                                        use_lookup_df, on_off_df, acres_df,
                                                                        BE_sum_PCT_interval, skip_non_ag_adjustment,
                                                                        use_direct_only_conus_ag,
