...
def main():
    ...
    final_order = ['Unnamed: 0', 'EntityID', 'STATEFP', 'VALUE_0', 'VALUE_30', 'VALUE_42', 'VALUE_60',
                     ...
                     'VALUE_1499', 'VALUE_1500', 'STATE', 'PCT_Methomyl AA', 'Acres', 'State direct msq', 'Drift_PCT',
                     'State msq adjusted by PCT', 'Total outside species range', 'Min in Species range',
...
def check_order(list_csvs, order_col):
    for csv in list_csvs:
        ...
        check_order_pct = [col for col in order_col if col.startswith ('PCT')]
        index_pct_check =  order_col.index(check_order_pct[0])
        pct_col = [col for col in df.columns.values.tolist() if col.startswith ('PCT')]
        order_col.insert(index_pct_check, pct_col[0])
        order_col.remove(check_order_pct[0])
        dict_order = {}
        for i in order_col:
