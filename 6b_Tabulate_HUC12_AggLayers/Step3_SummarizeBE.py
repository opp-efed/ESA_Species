...
# Title- Generate BE summary table from master percent overlap tables
...
in_table = r'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TerrestrialGIS\Tabulated_NewComps\Range_Gap\HUC12\Agg_Layers\SprayInterval_IntStep_30_MaxDistance_1501\HUC12_SprayInterval_20170911_HUC12.csv'
...
sp_table_df['HUC_12'] = sp_table_df['HUC_12'].map(lambda x: str(x)).astype(str)
sp_table_df['HUC_12'] = sp_table_df['HUC_12'].map(lambda x: str(x) if len(x) == 12 else '0' + str(x))
