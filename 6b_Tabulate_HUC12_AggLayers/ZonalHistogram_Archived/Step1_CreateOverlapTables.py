...
raw_results_csv = r'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TerrestrialGIS\Results_NewComps\test_gap\HUC12' \
                  r'\Agg_Layers'
...
def use_by_species(use_df, final_header):
    ...
    # use_df['HUC_12'] = use_df['HUC_12'].map(lambda x: str(x) if len(x) == 12 else '0' + str(x))

    use_cols = use_df.columns.values.tolist()
    use_cols.remove('HUC_12')

    use_df.ix[:, use_cols] = use_df.ix[:, use_cols].apply(pd.to_numeric, errors='coerce')

    use_df = use_df.groupby('HUC_12').sum()
    use_df = use_df.reset_index()
    new_columns_col = use_df.columns.values.tolist()
    # new_columns_col[0] = 'HUC_12'

    out_col = ['HUC_12']
    [out_col.append(final_header + "_" + str(j.replace(',', ''))) for j in new_columns_col if not j.startswith('HUC')]
...
for folder in list_folders:
    ...
    for csv in list_csv:
        ...
        acres_for_calc = acres_for_calc.groupby('HUC_12').sum()
        acres_for_calc = acres_for_calc.reset_index()
        ...
        if not overwrite_inter_data and os.path.exists(out_use_pixel_by_huc12):
            ...
            use_df_transformed = use_by_species(df_use, final_col_header)
    ...
    if not overwrite_inter_data and os.path.exists(out_csv):
        ...
        use_cols = out_df.columns.values.tolist()
        use_cols.remove('HUC_12')
        out_df.ix[:, use_cols] = out_df.ix[:, use_cols].apply(pd.to_numeric, errors='coerce')
        out_df = out_df.groupby('HUC_12').sum()
        out_df = out_df.reset_index()

        f_out_df = pd.merge(base_huc, out_df, on='HUC_12', how='left')

        f_out_df.drop_duplicates(inplace=True)
        [f_out_df.drop(m, axis=1, inplace=True) for m in f_out_df.columns.values.tolist() if m.startswith('Unnamed')]
        f_out_df.fillna(0, inplace=True)
        f_out_df.to_csv(out_csv)
