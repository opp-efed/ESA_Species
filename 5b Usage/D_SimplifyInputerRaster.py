...
in_directory_species_grids = 'L:\ESA\UnionFiles_Winter2018\Range\SpComp_UsageHUCAB_byProjection_2\Grid_byProjections_Combined'
# in_directory_species_grids = r'C:\Users\Admin\Documents\Jen\Workspace\StreamLine\ESA\UnionFiles_Winter2018\Range\SpComp_UsageHUCAB_byProjection' \
        ...
        for spe_raster in sp_list:
            ...
            if spe_raster.split("_")[1] in skip_species:
                ...
                if not arcpy.Exists(out_folder + os.sep + spe_raster):
                    ...
                    if not arcpy.Exists(out_folder_nibble + os.sep + spe_raster):
                        ...
                        try:
                            ...
                            arcpy.BuildRasterAttributeTable_management(out_folder_nibble + os.sep + spe_raster)
                        except Exception as error:
                            print('Error was {0} elapse time {1}'.format(error.args[0],
                    ...
                    try:
                        ...
                        att_df_org = pd.read_csv(
                            in_directory_species_grids + os.sep + folder + os.sep + spe_raster + '_att.csv')
                        col_header = pd.read_csv(
                            in_directory_species_grids + os.sep + folder + os.sep + spe_raster + '_lookup_rasters.csv')
                        ...
                        for v in att_df_org.columns.values.tolist():
                            if v in [u'Rowid', u'VALUE', u'COUNT']:
                                ...
                                new_col = \
                                col_header.loc[col_header['Default output header'] == v, 'Desired output header'].iloc[0]
                                out_col.append(new_col)
                            else:
                                out_col.append(v)
                        att_df_org.columns = out_col
                        att_df_org['VALUE_in'] = att_df_org['VALUE_in'].map(lambda x: x).astype(int)
                        ...
                        arcpy.Delete_management("in_memory\\view")
                        arcpy.Delete_management("lyr")
                        table_location = (out_folder_nibble + os.sep + spe_raster + '_att_smooth.csv')
                        arcpy.TableToTable_conversion(table_location, 'in_memory', 'view')
                        # arcpy.MakeTableView_management(table_location, "view")

                        arcpy.MakeRasterLayer_management(out_folder_nibble + os.sep + spe_raster, 'lyr')

                        arcpy.AddJoin_management('lyr', 'VALUE', 'in_memory\\view', 'VALUE')
                        # Where clause for maketableview rather than table to conversion
                        # where = '"{0}_att_smooth.csv.VALUE_in" IS NOT NULL'.format(spe_raster)
                        where = '"view.VALUE_in" IS NOT NULL'

                        outExtract = ExtractByAttributes('lyr', where)
                        outExtract.save(out_folder_ext + os.sep + spe_raster)
                        arcpy.BuildRasterAttributeTable_management(out_folder_ext + os.sep + spe_raster)
                        ...
                        print('\nError was {0} elapse time {1}'.format(error.args[0],
                                                                     datetime.datetime.now() - start_raster))
                        if arcpy.Exists(out_folder + os.sep + spe_raster[:8]):
                            arcpy.Delete_management(out_folder + os.sep + spe_raster[:8])

                else:
                    print '\nAlready created file {0}'.format(out_folder + os.sep + spe_raster)
