...
def habitat_xwalk(df_melt_row, id_vars_melt):
    df_melt_row['EntityID'].fillna('None', inplace=True)
    df_melt_row = df_melt_row.loc[df_melt_row['EntityID'] != 'None']
    df_melt_row.drop('melt_var', axis=1, inplace=True)
    df_melt_row.ix[:, id_vars_melt] = df_melt_row.ix[:, id_vars_melt].apply(pd.to_numeric)
    ...
    hab_by_ent.pivot_table(rows='EntityID', cols="rn", values=0)  # pivot results into individual columns
    return hab_by_ent
...
for folder in list_dir:
    ...
    for csv in list_csv:
        ...
        parent_id_dem_col = [v for v in dem_df.columns.values.tolist() if v.startswith('ch_') or v.startswith('r_')]
        dem_df[parent_id_dem_col[0]] = dem_df[parent_id_dem_col[0]].map(lambda x: str(x).split('.')[0]).astype(str)
        dem_df['HUCID'] = dem_df[parent_id_dem_col[0]].map(lambda x: str(x).split('.')[0]).astype(str)
        c_dem_parent_id = dem_df[parent_id_dem_col[0]].values.tolist()
        ...
        merg_dem_par, dem_zones = merge_to_hucid(par_zone_df, dem_df, c_dem_parent_id, ['ZoneID', 'HUCID'], 'HUCID')
        ...
        out_elevation_working, id_var_list = parse_tables(merg_dem_par, sp_zone_dem_df)
        ...
        merg_hab_par, hab_zones = merge_to_hucid(par_zone_df, habitat_df, c_hab_parent_id, ['ZoneID', 'HUCID'], 'HUCID')

        sp_zone_hab_df = sp_zone_df[sp_zone_df['ZoneID'].isin(hab_zones)]
        out_habitat_working, id_var_list = parse_tables(merg_hab_par, sp_zone_hab_df)
        out_habitat_working = habitat_xwalk(out_habitat_working, id_var_list)
