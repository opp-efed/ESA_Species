...
# TODO - 1) SPlit double crop evenly across the two groups, then generate a tables for each crops from out by FIPS and ST
# 2) Calculate total us state from political boundary results then apply PCT to species overlap and assumed min and max
...
raw_results_csv = r'L:\results_intersect\Range\Agg_Layers'
...
if 'Range' in find_file_type or 'range' in find_file_type:
    ...
    look_up_fips = r'L:\L48_intersect.gdb'
    ...
    recode_path =r'L:\Workspace\UseSites\CDL_Reclass\recodes_step3.csv'

    file_type = 'R_'
    species_file_type = 'Range'
else:
    look_up_fc = r'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TerrestrialGIS\_MovedOneDrive\Union\CriticalHabitat' \
...
in_acres_list = [
    r'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TerrestrialGIS\_MovedOneDrive\Tables'
    r'\CH_Acres_by_region_20170208.csv',
    r'L:\Workspace\ESA_Species\Step3\ToolDevelopment\TerrestrialGIS\_MovedOneDrive\Tables'
    r'\R_Acres_by_region_20171204.csv']

find_file_type = raw_results_csv.split(os.sep)

out_root = r'L:\tabulated_intersect' + os.sep + species_file_type

if 'Agg_Layers' in find_file_type:
    out_results = out_root + os.sep + 'Agg_Layers'
else:
    out_results = out_root + os.sep + 'Indiv_Layers'
...
recode_df = pd.read_csv(recode_path)
recode_df["Value"]= recode_df["Value"].map(lambda x: str(x))
recode_df.fillna(-99, inplace = True)


recode_splits = recode_df.loc[recode_df['Split a']>0]
recode_list = recode_splits['Value'].values.tolist()
...
def split_dbl_crop(use_df):
    drop_cols = [t for t in use_df.columns.values.tolist() if not t.startswith('V')]
    drop_cols.remove('LABEL')
    [use_df.drop(j, axis=1, inplace=True) for j in drop_cols if j in drop_cols]
    use_df["LABEL"] = use_df["LABEL"].map(lambda x: str(x).astype(str))
    zone_cols = [t for t in use_df.columns.values.tolist() if t.startswith('V')]
    for recode in recode_list:
        value_sum_a = recode_splits.loc[recode_splits["Value"] == recode, "Split a"].iloc[0]
        value_sum_b = recode_splits.loc[recode_splits["Value"] == recode, "Split b"].iloc[0]
        for z in zone_cols:
            if value_sum_a > 0 and value_sum_b > 0:
                split_value = use_df.loc[use_df["LABEL"] == recode, z].iloc[0]
                c_value_a = use_df.loc[use_df["LABEL"] == str(value_sum_a).split(".")[0], z].iloc[0]
                c_value_b = use_df.loc[use_df["LABEL"] == str(value_sum_b).split(".")[0], z].iloc[0]
                use_df.loc[use_df["LABEL"] == str(value_sum_a).split(".")[0], z] = c_value_a + (split_value / 2)
            else:
                split_value = use_df.loc[use_df["LABEL"] == recode, z].iloc[0]
                c_value_a = use_df.loc[use_df["LABEL"] == str(value_sum_a).split(".")[0], z].iloc[0]
                use_df.loc[use_df["LABEL"] == str(value_sum_a).split(".")[0], z] = c_value_a + (split_value)
            
    parent_value = recode_df.loc[recode_df['Split a'] < 0]
    use_df_out = df_use.loc[df_use["LABEL"].isin(parent_value["Value"].values.tolist())]
    return use_df_out
...
for folder in list_folders:
    ...
    if len(list_csv) == 0:  # out use folder create but not runs complete
        ...
        for csv in list_csv:
            ...
            df_use_parent = split_dbl_crop(df_use)
            # TODO make a table for each use in thedf_use_parent by row, and splut by FIPS then by STATE
