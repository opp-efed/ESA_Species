...
# User input variable
inGDB = 'J:\Workspace\ESA_Species\CriticalHabitat\NAD_Final\Clams.gdb'
archiveGDB = 'J:\Workspace\ESA_Species\CriticalHabitat\NAD_Final\Clams.gdb'
# final field in fc
fieldlist = ["EntityID", "NAME", "Name_sci", "SPcode", "VIPCode", "Status"]
...
for i in duplist:
    ...
    with arcpy.da.UpdateCursor(out_final, fieldlist) as cursor:
        for row in cursor:
            row[1] = name
            row[2] = sci
            row[3] = spcode
