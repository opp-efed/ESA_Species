...
def load_data(current, removed, added):
    ...
    [c_df.drop(v, axis=1, inplace=True) for v in c_df.columns.values.tolist() if v.startswith('Unnamed')]
    c_df[str(entity_id_col_c)] = c_df[str(entity_id_col_c)].astype(str)
    final_cols = c_df.columns.values.tolist()
    bin_loc_start = final_cols.index((bin_col[0]))
    ...
    bin_loc_end = (final_cols.index((bin_col[len(bin_col) - 1]))) + 1
    ...
    removed_cols = update_columns(str(entity_id_col_r), str(entity_id_col_c), removed_cols)
    ...
    return c_df, remove_df, add_df, final_cols, bin_loc_start, bin_loc_end
...
def add_huc_assignments(row, look_df, add_df):
    ...
    entid = str(row[str(entity_id_col_c)])

    if entid in add_df[str(entity_id_col_c)].values.tolist():
    ...
    for v in ent_list:
        if collapse_new_HUCS_only:
            if v not in list_added_species:
                    ...
                    try:
                        ...
                        for x in current_bins:
                            ...
                            if out_value == x:
                                ...
                                update_value = max(out_value, x)  # coded bin values are hierarchical retain max value
                ...
                if huc_specific_assignments:
                    for t in spe_huc:
                        working_df.loc[working_df['Spe_HUC'] == t, ['Updated']] = 'HUC assignments are different'
...
df_current, df_remove, df_add, final_col, bin_start_index, bin_end_index = load_data(current_bin_table,
                                                                                     removed_huc_table, add_huc_table)
...
df_land = df_final.loc[:, :].copy()  # make a copy of the df based on the applied indexing to this point
