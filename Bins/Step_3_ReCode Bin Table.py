...
wide_bin_coded_name = 'UpdatedBins_20170504.csv'
...
# Bin 10'] are in a continuous are in order

def load_data(current):
    # VARS: current: current wide bin table in use
...
def check_reassigned_bin(row, df):
    ...
    bool_reassign = 'FALSE'
    ent_huc = row['Spe_HUC']
    lookup_huc_bins = df.loc[df['Spe_HUC'] == ent_huc]
...
def check_multi_huc(row, df):
    # VARS: row: row of data being updated (apply function to flag species with a reassigned bin code) df: working_df
    # with bin codes
    # DESCRIPTION: Flags species as Y  if they are found in multiple HUC2s
    # Try/Except capture an index error when making update
    # RETURN: updated values for column being updated by apply function
    entid = row[str(entity_id_col_c)]

    filter_df = df.loc[df[str(entity_id_col_c)] == entid]
    if len(filter_df) > 1:
        return 'Y'
...
df_bins, df_bins_col = load_data(wide_bin_coded)
...
print 'Check output table for errors in the reassigned, multi huc, sur huc and dd species columns'
