...
chemical = 'Methomyl'  #chemical name
...
for state in states:
    ...
    genclasses = list(set(state_df['AgClass'].values.tolist()))
    for genclass in genclasses:
        # print genclass
        genclass_df = state_df.loc[(in_df['AgClass'] == genclass) & (state_df['Acreage'] != -9999)].copy()
        if len(genclass_df) > 0:
            out_df = weight_pct(genclass_df, out_df, state, genclass, 'AgClass')
