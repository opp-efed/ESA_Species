...
# TODO remove function for hab and elevation and just send the working elevation and habitat df through the habitat
# function
...
out_political = r'E:\Workspace\StreamLine\ESA\Tabulated_UsageHUCABHabitat\PolBoundaries'
#
# in_directory_csv = r'E:\Workspace\StreamLine\ESA\Results_HabEle\NL48\Range\Agg_Layers'
# out_path = r'E:\Workspace\StreamLine\ESA\Tabulated_UsageHUCABHabitat'
# out_political = r'E:\Workspace\StreamLine\ESA\Tabulated_UsageHUCABHabitat\PolBoundaries'

run_habitat = True  # type: bool
run_elevation = False  # type: bool
run_aqu = True  # type: bool

adjustment_inputs = r'E:\Workspace\StreamLine\ESA\UnionFiles_Summer2019\Range\input tables'
elevation_adjustments = adjustment_inputs + os.sep + "Elevation_Summary_20180809.csv"
...
skip_regions = []
# 'r_amphib', 'r_birds', 'r_clams', 'r_conife',  'r_crusta', 'r_ferns','r_fishes', 'r_flower','r_insect','r_lichen', 'r_mammal',  'r_reptil','r_snails']
skip_species = [ 'r_mammal', ]
...
folder_skip = ['CONUS_carbaryl_AA_190301_dev_euc', 'CONUS_carbaryl_AA_190507_dev_euc',
               ...
               'CONUS_methomyl_AA_Ag_190507_dev_euc', ]  # name of use folder with results to skip over
...
def no_adjust_loop(out_df, final_df, cnty_all, sta_all):
    # generate outs with no adjustments
...
def adjust_elevation(out_df, adjust_path, final_df, cnty_all, sta_all):
    ...
    if 'VALUE' in val_cols:  # drop the 'VALUE' col or zone identifier
        val_cols.remove('VALUE')
    # get dem col
    dem_col = [q for q in out_df.columns.values.tolist() if str(q).startswith('dem')]
    # set dem col as an int
    out_df.ix[:, dem_col[0]] = out_df.ix[:, dem_col[0]].map(lambda z: z).astype(int)
    # lookup table with elevations for a species to include
    adjust_df = pd.read_csv(adjust_path)
    index_cols = [d for d in adjust_df.columns.values.tolist() if d.startswith('Unnamed')]
    if len(index_cols) > 0:
        adjust_df.drop(index_cols, axis=1, inplace=True)  # drops index rows
    # set entity id as str
    ...
    e_adjust = out_df.loc[out_df['EntityID'].isin(sp_to_adjust)].copy()  # copy species to adjust
    # do the overlap adjust to only include elevation between the min and max from look-up
    for e in sp_to_adjust:
        if e in e_adjust['EntityID'].values.tolist():
            print e  # print species number
            min_v = adjust_df.loc[(adjust_df['EntityID'] == e, 'Min Elevation GIS')].iloc[0]  # get min elevation
            max_v = adjust_df.loc[(adjust_df['EntityID'] == e, 'Max Elevation GIS')].iloc[0]  # get max elevation
            print min_v, max_v  # print min and max
            # extract all row for v(species) with found between the elevations
            w_df = e_adjust.loc[(e_adjust['EntityID'] == e) & (e_adjust[dem_col[0]] <= int(max_v)) & (
    ...
    final_df = pd.concat([final_df, out_ele_loop])  # concat if looping on tables
    ...
    cnty_all = pd.concat([cnty_all, df_cnty])  # concat if looping on tables
    col_order = [e for e in df_cnty if e != 'GEOID']

    # # sums each the distance value cols for the species by county across rows
    df_state = df_cnty.groupby(['EntityID', 'STATEFP', 'STUSPS'])[val_cols].sum().reset_index()
    del df_cnty  # clears memory
    df_state = df_state.reindex(columns=col_order)  # reindex col order
    sta_all = pd.concat([sta_all, df_state])  # concat if looping on tables
    del df_state  # clears memory
    # returns e_working will have both the habitat and elevation column to so habitat can be adjusted
...
def adjust_elv_habitat(e_h_working, out_df, hab_sp_to_adjust, hab_df_for_adjust, out_final, cnty_all, sta_all):
    # Adjusts for elevation and habitat based on input file
    val_cols = [i for i in out_df.columns.values.tolist() if str(i).startswith('VALUE')]
    if 'VALUE' in val_cols:
        val_cols.remove('VALUE')
    # files the species that have habitats to adjust to; input already has elevation adjusted e_h_working
    # species that won't be adjusted are saved to working df
    e_h_working = e_h_working.loc[~e_h_working['EntityID'].isin(hab_sp_to_adjust)].copy()
    # species to be adjusts
    e_h_adjust = e_h_working.loc[e_h_working['EntityID'].isin(hab_sp_to_adjust)].copy()
    # get habitat col
    hab_col = [q for q in out_df.columns.values.tolist() if str(q).startswith('Habit') or str(q).startswith(
        'gap') or str(q).startswith('2011')]
    # set habitat values to str
    out_df[hab_col[0]] = out_df[hab_col[0]].map(lambda r: r.split('.')[0]).astype(str)
    # extract just the hbaita classes to be included
    for h in hab_sp_to_adjust:
        if h in e_h_adjust['EntityID'].values.tolist():
            hab_cat = hab_df_for_adjust[h].values.tolist()
            w_eh_df = e_h_adjust.loc[(e_h_adjust['EntityID'] == h) & (e_h_adjust[hab_col[0]].isin(hab_cat))].copy()
...
def load_csv(in_path, spe_look_up, region):
    ...
    spe_look_up.ix[:, 'VALUE'] = spe_look_up.ix[:, 'VALUE'].map(lambda n: str(n).split('.')[0]).astype(str)
    ...
    for df in pd.read_csv(in_path, engine='c', chunksize=chunksize, iterator=True, low_memory=True):  # type: df
        ...
        if len(df) > 0:
            ...
            df, dem, hab = add_spe_att(spe_look_up, df, region)
            if pp == 1:  # if this is the first loop the current df is saved as the out_all df
                ...
                if not region == 'AK':  # no habitat file in AK so it can't have the hab cal
                    ...
                    out_all = out_all.groupby(['EntityID', 'GEOID', 'STUSPS', 'STATEFP', 'HUC2_AB', hab], sort=False,
                                              as_index=False).sum()
                else:
            ...
            out_all, dem, hab = add_spe_att(spe_look_up, df, region)
    print ("    Memory usage {0}".format(mem_usage(out_all)))
...
def add_spe_att(spe_look_up, raw_df, region):
    ...
    # extract the dem col from attributes
    dem_col = [t for t in spe_look_up.columns.values.tolist() if str(t).startswith('dem')][0]
    ...
    if not region == 'AK':  # AK doesn't have hab
        # out_sp_and_huc =  merge_par.groupby(['EntityID', 'GEOID','STUSPS', 'STATEFP', 'HUC2_AB', hab_col, dem_col],
        # sort=False, as_index= False).sum()
        out_sp_and_huc = merge_combine.groupby(['EntityID', 'GEOID', 'STUSPS', 'STATEFP', 'HUC2_AB', hab_col],
                                               sort=False, as_index=False).sum()
    else:
        # out_sp_and_huc =  merge_par.groupby(['EntityID', 'GEOID','STUSPS', 'STATEFP', 'HUC2_AB', dem_col],
        # sort=False, as_index= False).sum()
...
def export_tables(loop_sp_table, folder, csv, time, out_pol_folder_st, out_pol_folder_cny, region):
    global elev_hab_working, hab_df, hab_sp_adjust, hab_df, hab_sp_adjust
    ...
    out_ele = pd.DataFrame(columns=[])  # type: DataFrame
    out_ele_cnty = pd.DataFrame(columns=[])  # type: DataFrame
    out_ele_state = pd.DataFrame(columns=[])  # type: DataFrame

    out_hab_ele = pd.DataFrame(columns=[])  # type: DataFrame
    out_hab_ele_cnty = pd.DataFrame(columns=[])  # type: DataFrame
    out_hab_ele_state = pd.DataFrame(columns=[])  # type: DataFrame
    ...
    if len(loop_sp_table) > 0:
        ...
        if run_aqu:  # export aquatic tables if boolean set to turn to include  aquatic tables
            if not os.path.exists(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                                  csv.replace('.csv', '_HUC2AB.csv')):
                # Generates aquatic tables
                aqu = export_aquatics(loop_sp_table, aqu)
                # saves to csv
                aqu.to_csv(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                           csv.replace('.csv', '_HUC2AB.csv'))
                print ('  Exported {0}'.format(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                                               csv.replace('.csv', '_HUC2AB.csv')))
        ...
        if not os.path.exists(out_pol_folder_st + os.sep + csv.replace('.csv', '_noadjust.csv')):
            ...
            no_adjust_no, no_adjust_cnty, no_adjust_state = no_adjust_loop(loop_sp_table, no_adjust_no,
                                                                           ...
                                                                           no_adjust_state)
            # saves to csv
            no_adjust_no.to_csv(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                                csv.replace('.csv', '_noadjust.csv'))
        ...
        # export tables with elevation adjustments made if Boolean for elevation is set to true
        if run_elevation:
            if not os.path.exists(out_pol_folder_st + os.sep + csv.replace('.csv', '_adjEle.csv')):
                # generates table with elevation adjustments, and saves a working df used when making elevation and
                # habitat adjustments
                ...
                # saves to csv
                out_ele.to_csv(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                               csv.replace('.csv', '_adjEle.csv'))
            ...
            else:  # elevation table was already saved but need the working df for adjusting by hab and elevations
                elev_hab_working, out_ele, out_ele_cnty, out_ele_state = adjust_elevation(
                    loop_sp_table, elevation_adjustments, out_ele, out_ele_cnty, out_ele_state)

                print("Previously generated elevation output and species table took {0}".format
                      ((datetime.datetime.now() - time)))
        ...
        if region != 'AK':  # no hab files in AK
            if run_habitat:  # generated habitat tables if habitat boolean set to true
                if not os.path.exists(out_pol_folder_st + os.sep + csv.replace('.csv', '_adjHab.csv')):
                    ...
                    # saves to csvs
                    out_habitat.to_csv(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                                       csv.replace('.csv', '_adjHab.csv'))
            ...
            if run_habitat and run_elevation:
                # generated habitat and elevation adjust tables tables if habitat and elevation booleans set to true
                if not os.path.exists(out_pol_folder_st + os.sep + csv.replace('.csv', '_adjEleHaB.csv')):
                    # load data into tables and generates working files
                    out_hab_ele, out_hab_ele_cnty, out_hab_ele_state = adjust_elv_habitat(elev_hab_working,
                                                                                          loop_sp_table, hab_sp_adjust,
                                                                                          hab_df, out_hab_ele,
                                                                                          out_hab_ele_cnty,
                                                                                          out_hab_ele_state)
                    # saves to csvs
                    out_hab_ele.to_csv(out_path + os.sep + 'Agg_Layers' + os.sep + folder + os.sep +
                                       csv.replace('.csv', '_adjEleHab.csv'))
                    out_hab_ele_cnty.to_csv(out_pol_folder_cny + os.sep + csv.replace('.csv', '_adjEleHab.csv'))
                    out_hab_ele_state.to_csv(out_pol_folder_st + os.sep + csv.replace('.csv', '_adjEleHab.csv'))
                    ...
                    # clears memory
                    del out_hab_ele, out_hab_ele_cnty, out_hab_ele_state, elev_hab_working, hab_sp_adjust, hab_df
                    print("Previously generated habitat/elevation output: species table took {0}".format(
                        (datetime.datetime.now() - time)))
            else:
    ...
    for folder in list_dir_results:  # loops over folders with results
        ...
        if folder in folder_skip:  # skip folder passed on user parameters
            ...
            if region not in skip_regions:
                ...
                # loops on each csv adds species, huc, political boundary, habitat and dem information from parent fc
                ...
                for csv in list_csv:
                    ...
                    if run_habitat and not run_elevation:
                        final_csv = out_pol_folder_st + os.sep + csv.replace('.csv', '_adjHab.csv')
                    elif run_elevation and not run_habitat:
                        final_csv = out_pol_folder_st + os.sep + csv.replace('.csv', '_adjEle.csv')
                    else:
                        final_csv = out_pol_folder_st + os.sep + csv.replace('.csv', '_adjEleHab.csv')
                    ...
                    if os.path.exists(final_csv):  # LAST CSV
                        ...
                        if not os.path.exists(out_pol_folder_st + os.sep + csv):
                            ...
                            if os.path.exists(look_up_fc + os.sep + region_lookup[0] + os.sep + lookup_spe[0]):
                                ...
                                loop_sp_table, hab_col, dem_col = load_csv(in_csv_path, spe_att, region)

                                if region != 'AK':  # No hab files in AK
                                    final_col_order_att = final_col_order + [hab_col]
                                    # TODO WILL NEED TO ADD DEM COL
                                    # final_col_order_att = final_col_order + [hab_col, dem_col]
                                else:
                                    final_col_order_att = final_col_order
                                    # final_col_order_att = final_col_order + [dem_col]
