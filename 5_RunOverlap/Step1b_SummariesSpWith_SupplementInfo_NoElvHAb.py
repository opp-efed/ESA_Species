...
# in_directory_csv = r'E:\Workspace\StreamLine\ESA\Results_HUCAB\NL48\CriticalHabitat\Agg_Layers'
in_directory_csv = r'E:\Workspace\StreamLine\ESA\Results_OnOff\NL48\Range\Agg_Layers'
# look up directory
# look_up_fc_ab = r'E:\Workspace\StreamLine\ESA\UnionFiles_Summer2019\CriticalHabitat\Lookup_CH_Clipped_Union_CntyInter_HUC2ABInter_20190812'
look_up_fc_ab =  r'E:\Workspace\StreamLine\ESA\UnionFiles_Summer2019\Range\Lookup_R_Clipped_Union_CntyInter_HUC2ABInter_20190812'
out_path = r'E:\Workspace\StreamLine\ESA\Tabulated_OnOff'
out_poltical = r'E:\Workspace\StreamLine\ESA\Tabulated_OnOff\PolBoundaries'
look_up_id = 'HUCID'  # aCan be HUCID, InterID or ZoneID
...
def split_csv_chucks(in_path, look_up):
    ...
    look_up.ix[:, look_up_id] = look_up.ix[:, look_up_id].map(lambda z: str(z).split('.')[0]).astype(str)
    all_zones_sp = look_up[look_up_id].values.tolist()
    ...
    # if len(df) > 0:
        ...
        look_up_huc = look_up[look_up[look_up_id].isin(list_zones)]  # filter lookup to just zones in current output

        # merges working table with HUCID field or other look up columns
        merge_par = pd.merge(c_csv, look_up_huc, how='outer', left_on='VALUE', right_on=look_up_id)
        ...
        if run_aqu:
            ...
            merged_df_huc = merge_par[['EntityID', 'STUSPS'] + val_cols].copy()
            # out_sp_table_huc = merged_df_huc.groupby(['EntityID', 'STUSPS', 'STATEFP', 'HUC2_AB'])[
            #     val_cols].sum().reset_index()

            out_sp_table_huc = merged_df_huc.groupby(['EntityID', 'STUSPS', ])[
                val_cols].sum().reset_index()
