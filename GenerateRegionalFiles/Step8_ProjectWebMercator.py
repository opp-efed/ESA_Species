...
outgdb = r'J:\Workspace\ESA_Species\ForCoOccur\Composites\CurrentComps\WebApp\Composites\NL48_R_SpGroup_Composite_Web.gdb'
...
##TODO add check to make sure max composite is taken and set up user questions
                ...
                if desireddatum == ORGdataum:
                    ...
                    if not arcpy.Exists(outfc):
                        if not arcpy.Exists(tempgeo):
                            ...
                            arcpy.Project_management(infc, tempgeo, ORGsr)
                        if not arcpy.Exists(geodesired):
                            print "Generating temporary final geographic projected file... "
                            arcpy.Project_management(tempgeo, geodesired, Coord)
                        print "Generating final projected FC..."
                        arcpy.Project_management(geodesired, outfc, descoord_sys)
